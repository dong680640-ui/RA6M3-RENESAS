...
void R_IRQ_Setting()
{
    ...
    ///////////////////////// YOU MUST EDIT CODE HERE ///////////////////////////////////
     ...
     R_ICU->IELSR_b[0].IELS = (uint32_t)0x0C; // ICU Event Link Select (Refer to the Table 14.4) (Port_IRQ11)
     R_ICU->IELSR_b[1].IELS = (uint32_t)0x0D; // ICU Event Link Select (Refer to the Table 14.4) (Port_IRQ12)
     R_ICU->IELSR_b[2].IELS = (uint32_t)0x0E; // ICU Event Link Select (Refer to the Table 14.4) (Port_IRQ13)
     R_ICU->IELSR_b[3].IELS = (uint32_t)0x0F; // ICU Event Link Select (Refer to the Table 14.4) (Port_IRQ14)
     ...
     R_PFS->PORT[0].PIN[8].PmnPFS_HA_b.ASEL = (uint16_t)0U; // Port m/n Pin Function Select: Analog Input
     R_PFS->PORT[0].PIN[8].PmnPFS_HA_b.ISEL = (uint16_t)1U; // Port m/n Pin Function Select: IRQ Input Mode
     R_PFS->PORT[0].PIN[9].PmnPFS_HA_b.ASEL = (uint16_t)0U; // Port m/n Pin Function Select: Analog Input
     R_PFS->PORT[0].PIN[9].PmnPFS_HA_b.ISEL = (uint16_t)1U; // Port m/n Pin Function Select: IRQ Input Mode
...
void R_IRQ11_ISR(external_irq_callback_args_t *p_args)
{
    ...
    ///////////////////////// YOU MUST EDIT CODE HERE ///////////////////////////////////
    R_ICU->IELSR_b[0].IR = (uint32_t)0U; // ICU Status Flag Clear (You must only write '0'.)

    num1 = (uint8_t)((num1 + 1) % 10);
    print_data[0] = number[num1];
    /////////////////////////////////////////////////////////////////////////////////////
}
...
void R_IRQ12_ISR(external_irq_callback_args_t *p_args)
{
    FSP_PARAMETER_NOT_USED (p_args);
    ///////////////////////// YOU MUST EDIT CODE HERE ///////////////////////////////////
    R_ICU->IELSR_b[1].IR = (uint32_t)0U; // ICU Status Flag Clear (You must only write '0'.)

    num2 = (uint8_t)((num2 + 1) % 10);
    print_data[1] = number[num2];
    /////////////////////////////////////////////////////////////////////////////////////
}
...
void R_IRQ13_ISR(external_irq_callback_args_t *p_args)
{
    FSP_PARAMETER_NOT_USED (p_args);
    ///////////////////////// YOU MUST EDIT CODE HERE ///////////////////////////////////
    R_ICU->IELSR_b[2].IR = (uint32_t)0U; // ICU Status Flag Clear (You must only write '0'.)
...
void R_IRQ14_ISR(external_irq_callback_args_t *p_args)
{
    FSP_PARAMETER_NOT_USED (p_args);
    ///////////////////////// YOU MUST EDIT CODE HERE ///////////////////////////////////
    R_ICU->IELSR_b[3].IR = (uint32_t)0U; // ICU Status Flag Clear (You must only write '0'.)

    num1 = 0;
    num2 = 0;
    result = 0;
    for (int i = 0; i < 4; i++) {
        print_data[i] = number[0];
    }
    /////////////////////////////////////////////////////////////////////////////////////
}
